...
In \emph{graph}, we define the Graph Window Query which summarize the vicinity of each vertex.
The query utilize the both distance neighborhood and comparison neighborhood to facilitate both
...
In \emph{sequenced data}, we propose a $k$-Sketch Query to summarize a subject's history. The $k$-Sketch query is on
the basis of a nested \emph{distant} and \emph{comparison} neighborhoods based pattern called \emph{rank-aware streak}.
%
...
we also investigate on supporting efficient Graph Window Query processing.  We propose
...
In the trajectory analysis, an important
...
On the technical side, we study how to efficiently processing GCMP query 

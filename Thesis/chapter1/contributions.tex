...
\doubted{Particularly, we propose and enrich the neighborhood analytics in the
three data domains. In \emph{graph}, we identify two types
of the window queries: \emph{k-hop} window is based on the \emph{distance} neighborhood
and \emph{topological} window is based on the \emph{comparison} neighborhood. In \emph{time series}, we 
leverage the nested \emph{distant} and \emph{comparison} neighborhoods to design the rank-aware streak discovery query.
In \emph{trajectory}, we generalize the movement pattern detection query utilizing the \emph{comparison} 
neighborhood.}

...
which are different from Graph Window Queries.
In a general model, graph aggregation contains three stages: (1) partition
graph based on attributes of vertex (and/or edges), (2) aggregate each
partition to form an Aggregated Node, and (3) link each aggregate node to
form an Aggregated Graph. An illustration of the Graph Aggregation is shown 
in Figure~\ref{fig:rw-agg-window} (b). In the first stage, the input graph
is partitioned based by the ``Gender'' attribute of vertexes. In the second
stage, two aggregated nodes are formed, i.e., $M$ which contains nodes $A,D,E$
and $F$ which contains nodes $B,C,F$. In the third stage, the links between 
$M$ and $F$ are added, with the count on each links. 
Graph Window Queries perform graph analytics from an different angle. In GWQ,
the neighborhood structure of each vertex form overlapping partitions. Then,
analytics are computed over each neighborhood structure. In the same example 
of Figure~\ref{fig:rw-agg-window} (c), the neighborhood structure of $B$ and $E$
are highlighted.  Due to the different
nature of Graph Aggregation and GWQ, they could not model each other.

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{chapter2/graphagg-window}
...
proposed~\cite{yu2010graph,cheng2012k}. 
...
networks however they do not consider efficient processing of data analytics inside
...
each vertex's neighborhoods. Their technique is to
leverage the shared components among vertexes' neighborhoods structure which can be 
mined via Frequent-Pattern Mining Tree (FP-Tree). Then, an overlay graph is 
constructed to record the shared components.
Technically, EAGR run in iterations and starts with the vertex-neighborhood mapping
...
The main drawback of EAGR is its high demands of memory resources 
on the overlay construction. In terms of memory, EAGR assume the 
...
largely reduced.  Another drawback of EAGR is its high computation cost since it requires to sort all vertexes 
...
We tackle these drawbacks by adopting a hash based approach that clusters each vertex based
...
to be resided in memory. In order to reduce the repetitive computation of vertex's neighborhood,

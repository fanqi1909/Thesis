\begin{abstract}
As the increasing variety and volume of the data 
...
\emph{neighborhood analytics} which eliminates the sorting requirement.
We propose three interesting domain-specific queries on the basis
of two simple neighborhood functions.
...
the windows of each vertex and reuse the shared components during the query processing, 
...
by grouping temporally nearby events; Subsequently, streaks with the same size are 
grouped to generate their ranks. A $k$-Sketch query then selects $k$ ranked-streaks which best summarize
a subject's history.
...
Third, we propose the General Co-Movement Pattern (GCMP) in trajectory databases. A GCMP
